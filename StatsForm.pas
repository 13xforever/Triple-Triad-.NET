...
var
  ...
  _gs: TGameInfo;
begin
  if UseMT and Thinking then
    for i := 1 to 5 do
      begin
        if Threads[i].Completed or not Threads[i].Initialized then
          continue;
           
        t := 0;
        for j := 1 to 3 do
          t := t + Threads[i].LocalCurHandStat[j];
        for j := 0 to 3 do
          begin
            id := IntToStr(i * 10 + j);
            if (j > 0) and (t > 0) and (Threads[i].LocalCurHandStat[j] > 0) then
              tmp := FloatToStrF(Threads[i].LocalCurHandStat[j] * 100.0 / t, ffGeneral, 3, 1) + '%'
            else if (Threads[i].LocalCurHandStat[j] > 0) then
              tmp := IntToStr(Threads[i].LocalCurHandStat[j])
            else
              tmp := '-';
      ...
      begin
        ...
        for i := 1 to 3 do
          for j := 1 to 3 do
            begin
              id := IntToStr(i * 10 + j);
              if _gs.Field_Cell[i, j].Card.Used then
                begin
                  if _gs.Field_Cell[i, j].Card.Our then
                    (FindComponent('color' + id) as TShape).Brush.Color := clPlayer
                  else
                    (FindComponent('color' + id) as TShape).Brush.Color := clOpponent;
                  (FindComponent('l' + id) as TLabel).Caption := StringReplace(_gs.Field_Cell[i, j].Card.ID, '&', '&&', [])
                end
              else
                begin
